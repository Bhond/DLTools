...
public abstract class XMLParserUtil {
    ...
    public static void writeObjectType(XMLStreamWriter xmlsw, Object obj) throws XMLStreamException {
        xmlsw.writeStartElement(XMLStrCst.DATA_TYPE);
    ...
    public static void writeObjectValue(XMLStreamWriter xmlsw, Object obj) throws XMLStreamException {
        xmlsw.writeStartElement(XMLStrCst.VALUE);
    ...
    public static void writeNDArrayElement(XMLStreamWriter xmlsw, String arrName, INDArray arr) throws XMLStreamException {
        ...
        xmlsw.writeStartElement(XMLStrCst.SHAPE);
            XMLParserUtil.writeTypedData(xmlsw, XMLStrCst.X_AXIS_LENGTH, arr.getShape().getXAxisLength());
            XMLParserUtil.writeTypedData(xmlsw, XMLStrCst.Y_AXIS_LENGTH, arr.getShape().getYAxisLength());
            XMLParserUtil.writeTypedData(xmlsw, XMLStrCst.Z_AXIS_LENGTH, arr.getShape().getZAxisLength());
        xmlsw.writeEndElement();
        // Write data
        xmlsw.writeStartElement(XMLStrCst.DATA);
        int count = 0;
        ...
        while (xmlsr.hasNext() && (xmlsrType != XMLStreamReader.END_ELEMENT || localName != tagName)){
            ...
            switch (xmlsrType)
            {
                case XMLStreamReader.START_ELEMENT:
                    ...
                    switch (localName)
                    {
                        case XMLStrCst.DATA_TYPE:
                            type = xmlsr.getText();
                            break;
                        case XMLStrCst.VALUE:
                            switch (type)
                            {
                                case XMLStrCst.INTEGER:
                                    res = readInt(xmlsr);
                                    break;
                                case XMLStrCst.DOUBLE:
                                    res = readDouble(xmlsr);
                                    break;
                                case XMLStrCst.BASE_NDARRAY:
    ...
    public static Shape readShape(XMLStreamReader xmlsr) throws XMLStreamException {
        ...
        // xmlsr.hasNext() condition avoid reading a line that doesn't exist due to corruption of the file
        // </Data> is the last tag of the NDArray, no matter the array's name
        while (xmlsr.hasNext() && (xmlsrType != XMLStreamReader.END_ELEMENT || localName != XMLStrCst.SHAPE)){
            xmlsrType = xmlsr.next();
            switch (xmlsrType)
            {
                case XMLStreamReader.START_ELEMENT:
                    ...
                    switch (localName)
                    {
                        case XMLStrCst.X_AXIS_LENGTH:
                            xAxisLength = (int) XMLParserUtil.readTypedData(xmlsr, XMLStrCst.X_AXIS_LENGTH);
                            break;
                        case XMLStrCst.Y_AXIS_LENGTH:
                            yAxisLength = (int) XMLParserUtil.readTypedData(xmlsr, XMLStrCst.Y_AXIS_LENGTH);
                            break;
                        case XMLStrCst.Z_AXIS_LENGTH:
                            zAxisLength = (int) XMLParserUtil.readTypedData(xmlsr, XMLStrCst.Z_AXIS_LENGTH);
                            break;
                    }
                    break;
                case XMLStreamReader.END_ELEMENT:
    ...
    public static double[] readDoubleArray(XMLStreamReader xmlsr) throws XMLStreamException {
        ...
        // xmlsr.hasNext() condition avoid reading a line that doesn't exist due to corruption of the file
        // </Data> is the last tag of the NDArray, no matter the array's name
        while (xmlsr.hasNext() && (xmlsrType != XMLStreamReader.END_ELEMENT || localName != XMLStrCst.DATA)){
            xmlsrType = xmlsr.next();
            switch (xmlsrType)
            {
                case XMLStreamReader.CHARACTERS:
